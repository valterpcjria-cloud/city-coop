...
export async function POST(
    ...
    try {
        ...
        const { data: gestor } = await adminSupabase.from('gestors').select('id').eq('user_id', user.id).maybeSingle() as any
        const { data: manager } = await adminSupabase.from('managers').select('id').eq('user_id', user.id).maybeSingle() as any
        ...
        let turmaQuery = adminSupabase.from('classes').select('id, school_id').eq('id', id)

        if (!isGestor) {
            // 2. Se não é gestor, deve ser o professor da turma
            const { data: teacher } = await adminSupabase.from('teachers').select('id').eq('user_id', user.id).maybeSingle() as any
            ...
            console.log('[API_CLASS_STUDENTS] Access as Teacher:', teacher.id)
            turmaQuery = turmaQuery.eq('teacher_id', teacher.id)
        }
        ...
        for (const studentData of students) {
            if (studentData.id) {
                ...
                const { data: currentStudent } = await adminSupabase
                    .from('students')
                    .select('school_id')
                ...
                let { data: existingStudent } = await adminSupabase
                    .from('students')
                ...
                if (!existingStudent) {
                    const { data: newStudent, error: createError } = await adminSupabase
                        .from('students')
                        .insert({
